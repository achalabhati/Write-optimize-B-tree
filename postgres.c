 ...
 */
//deepali
 void identify_insert(const char *);
static const char * cmd_type_g ;
static char * relation_name_insert ;
//deepali
...
static int exec_simple_query_select(const char *query_string);
void relation_name(char s[1000]);
/* ----------------------------------------------------------------
			 ...
			 const char *username)
{
	...
	for (;;)
	{
		...
		//deepali soc
				const char *new_query_string;
				const char* actual_table;
				static char * new_str_select;
				//deepali eoc
				switch (firstchar)
				{
					case 'Q':			/* simple query */
						{
							...
							{
								//const char * cmd_type;
								identify_insert(query_string);
								if(!strcmp("INSERT",cmd_type_g))
														//deepali eoc
								{
									relation_name(query_string);
									char* oldtable=relation_name_insert;
									char* tem1=strcat(relation_name_insert,"_1");
								 char s[100]="SELECT * FROM ";
								 new_str_select=s;
									//printf(new_str_select);
								char* tem2=strcat(new_str_select,relation_name_insert);
															...
															{
																char* newq_string;
																int i=strstr(query_string,oldtable);

															int l=strlen(relation_name_insert);
															int s=0;
															while(s<i)
															{
																newq_string[s]=query_string[s];
																s++;
															}
															int j=0;
															int k=strlen(oldtable)+i;
															while(i<i+l)
															{
																newq_string[i]=relation_name_insert[j];
																i++;
																j++;
															}
															k=strlen(oldtable)+i;
while(query_string[k]!='\0')
{

	newq_string[i]=query_string[k];
	i++;
	k++;
}
char a[1000]="insert into sid_1 valuse('123');";
newq_string=a;
exec_simple_query(newq_string);
}
														}
														else
...
static int
exec_simple_query_select(const char *query_string)
{
	CommandDest dest = whereToSendOutput;
	MemoryContext oldcontext;
	List	   *parsetree_list;
...
void identify_insert(const char *query_string)
{
	CommandDest dest = whereToSendOutput;
	MemoryContext oldcontext;
	List	   *parsetree_list;
	ListCell   *parsetree_item;
	bool		save_log_statement_stats = log_statement_stats;
	static const char *commandTag;
	bool		was_logged = false;
	bool		isTopLevel;
	char		msec_str[32];
   static char query_type[30];   //deepali
...
//	pgstat_report_activity(STATE_RUNNING, query_string);
	...
	foreach(parsetree_item, parsetree_list)
	{
		...
		commandTag = CreateCommandTag(parsetree);
		/*int i=0;
		while(commandTag[i]!='\0')
		{
			query_type[i]=commandTag[i];
			i++;
		}
		query_type[i]='\0';
*/
		cmd_type_g=commandTag;
	}
//return cmd_type_g;
	}
...
for(i=0;i<strlen(s);i++)
{
if(s[i]=='i'&&s[i+1]=='n'&&s[i+2]=='t'&&s[i+3]=='o')
{
int k=i+4;
while(s[k]==' ')
{
